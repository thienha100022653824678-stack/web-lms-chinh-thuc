...
- **Thứ tự hiện thực hóa:** Phase 0 ✅ → Phase 1 ✅ (⑦ migration tooling + CI schema-drift gate — repo tooling DONE 2026-07-15, `docs/V3_PHASE_1_MIGRATION_TOOLING.md`; baseline B `db pull` owner-only pending) → Phase 2 ✅ (① RLS + key tiering + RPC write path — repo DONE 2026-07-15, `docs/V3_PHASE_2_RLS_KEY_TIERING.md`; apply `migration_v3_rls_policies.sql` on B after canary = owner-pending) → Phase 3 ✅ (④ outbox backbone canonical + ⑤ worker reuse — repo DONE 2026-07-15, `docs/V3_PHASE_3_OUTBOX_BACKBONE.md`; apply `migration_v3_outbox_dead_letters.sql` on B = owner-pending) → Phase 4 ✅ (② unified opaque session + ③ server-minted device credential — repo DONE 2026-07-15, `docs/V3_PHASE_4_SESSION_UNIFY.md`; Portal lockstep = owner-merged PR proposal `docs/V3_PORTAL_PR_PROPOSAL_SESSION.md`) → Phase 5 ✅ (⑥ router split + edge read-only — repo DONE 2026-07-15, `docs/V3_PHASE_5_ROUTER_EDGE.md`; v1/v2 delegate to legacy router unchanged, code-only) → Phase 6 ✅ (⑪ structured logs + metrics + tracing — repo DONE 2026-07-15, `docs/V3_PHASE_6_OBSERVABILITY.md`; code-only) → Phase 7 ✅ (⑨ diagnostics dashboard + modular-FE pattern — repo DONE 2026-07-15, `docs/V3_PHASE_7_FE_MODULAR.md`; big admin-page rewrite deferred to behind-v3-gating) → Phase 8 ✅ (⑩ shared event-schema package — repo DONE 2026-07-15, `docs/V3_PHASE_8_TYPESCRIPT_MONOREPO.md`; full JS→TS + pnpm workspace deferred to behind-v3-gating) → Phase 9 ✅ (⑫ session-bound signed-URL + DRM opt-in — repo DONE 2026-07-15, `docs/V3_PHASE_9_SIGNED_URL_DRM.md`; DRM provider config = owner-pending) → Phase 10 ✅ (⑧ cleanup — repo safe-subset DONE 2026-07-15: drift columns formalized additively + banhmi4k dead code removed, `docs/V3_PHASE_10_CLEANUP.md`; destructive subset = owner-approval gate, NOT done by design).

- **Stage 1 + post-Stage-1 repo hardening (2026-07-16, branch `v3/research-20260715`):** audit-only verification of the 4 V3 migrations (NOT applied) — `docs/V3_STAGE1_FINAL_REPORT.md` (preflight PASS, 255→271 tests green, B1-B3 audited additive/idempotent, secrets status, preview read-only) + `docs/V3_STAGE1_RLS_READINESS_REPORT.md` (B4 readiness + rollback SQL, hard stop). Then repo-side hardening: (a) **secret hygiene** — untracked 3 git-tracked `.env.prod.*` files that carried a Vercel OIDC token (`git rm --cached`, kept on disk; `.gitignore` already matched via `.env*`); history rewrite + token rotation remain owner-pending (irreversible). Doc `docs/V3_SECRET_HYGIENE_ENV_UNTRACK.md`. (b) **login-RPC grants** — verified `handle_student_session_login` is ALREADY hardened on prod (PUBLIC/anon/authenticated EXECUTE=false, service_role=true; `docs/V3_SCHEMA_GAP_SQL_RESULTS.md` §Block 4d); migration `migration_handle_student_session_login_grants_hardening.sql` (commit a849362) already ships the REVOKE+GRANT and is an ancestor of this branch; locked with `tests/v3-session-login-grants.test.mjs`. Doc `docs/V3_AUTH_HARDENING_LOGIN_GRANTS.md`. (c) **rollback drill** — authored `docs/V3_PROPOSAL_7_ROLLBACK_DRILL.md` (3 drills: A runtime kill_switch flip ≤3s no-redeploy, B fail-closed on missing row/DB error, C reverse 4 additive migrations) + DB restore rehearsal + owner sign-off checklist; closes the Stage 1 pre-canary gap. Running the drill stays owner-only. (d) **preflight/postflight** — `scripts/v3/preflight-v3.sql` + `scripts/v3/postflight-v3.sql` (read-only baseline + post-apply GO/NO-GO checks) + `tests/v3-preflight-postflight.test.mjs` proving both are write-free. Suite **271/271**. All commits pushed; main + `v1-stable-20260713` untouched. Remaining = owner-only (apply 4 migrations on B, provision `SUPABASE_DB_URL_RO` + `SUPABASE_ANON_KEY`, decide `posts` ownership, merge Portal session PR, run the rollback drill, Phase 10 destructive approval). See `docs/V3_OWNER_PENDING_ACTIONS`/[[v3-owner-pending-actions]].
