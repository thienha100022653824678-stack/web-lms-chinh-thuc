...
import { resolveEffectiveLmsTenant } from "../utils/lms-tenant.js";

const expectedProductionRef = "aqozjkfwzmyfunqvcyjv";
const outputJson = process.argv[2] || "PRODUCTION_LMS_TENANT_DRY_RUN.json";
const outputMarkdown = process.argv[3] || "PRODUCTION_LMS_TENANT_DRY_RUN.md";
...
const supabase = createClient(url, key, {
  ...
  global: { headers: { "x-read-only-purpose": "lms-lms-tenant-dry-run" } }
});
...
let learningSiteColumnPresent = true;
{
  const withSite = await supabase
    .from("courses")
    .select("id,slug,title,active,sales_site,learning_course_slug,lms_tenant")
    .order("slug");
  if (withSite.error?.code === "42703" || /lms_tenant/i.test(withSite.error?.message || "")) {
    ...
    courses = (baseline.data || []).map((row) => ({ ...row, lms_tenant: null }));
  } else {
...
for (const course of courses) {
  ...
  try {
    proposed = await resolveEffectiveLmsTenant(course, {
      findCourseBySlug: async (slug) => bySlug.get(slug) || null
    });
    reason = course.lms_tenant
      ? "explicit"
      : isAlias
  ...
  } else if (course.lms_tenant && !["yeunauan", "yeubep"].includes(course.lms_tenant)) {
    deterministic = false;
    warning = "INVALID_EXISTING_LMS_TENANT";
  ...
  rows.push({
    ...
    proposed_lms_tenant: proposed,
    reason,
    deterministic,
    legacy_or_new: course.lms_tenant ? "new/explicit" : "legacy-compatible",
...
const artifact = {
  ...
  lms_tenant_column_present: learningSiteColumnPresent,
  counts,
...
const mdRows = rows.map((row) =>
  `| \`${row.course}\` | ${row.sales_site || "NULL"} | \`${row.canonical_target}\` | ${row.proposed_lms_tenant || "UNRESOLVED"} | ${row.reason} | ${row.explicit_write_required} | ${row.warning || "—"} |`
).join("\n");
const markdown = `# Production Dual LMS Dry-Run
