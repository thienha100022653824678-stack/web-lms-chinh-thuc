...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (dualEnabled) {
        await assertCourseInLmsTenant(supabase, course, tenant, { canonicalOnly: true });
      }
    ...
    if (req.method === "PUT") {
      ...
      if (!current) {
        return res.status(404).json({
          success: false,
          code: "COURSE_NOT_FOUND_IN_LMS",
          error: "Không tìm thấy progress"
        });
      }
      if (dualEnabled) {
        await assertCourseInLmsTenant(supabase, current.course_slug, tenant, { canonicalOnly: true });
      }
      ...
      if (!lesson || lesson.course_slug !== current.course_slug) {
        return res.status(409).json({
          success: false,
          code: "COURSE_LMS_TENANT_MISMATCH",
          error: "Lesson/progress không cùng canonical course"
        });
      }
      ...
      if (dualEnabled) {
        await auditLmsTenantOperation(supabase, {
          ...
          action: "lms_dual_progress_update",
