  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (isLmsAdminMultiSiteEnabled()) {
        await assertCourseInLearningSite(supabase, courseSlug, requestLearningSite(req), {
          canonicalOnly: true
        });
      }
    ...
    if (req.method === "POST") {
      ...
      if (action === "create") {
        ...
        if (multiSiteEnabled) {
          await assertCourseInLearningSite(supabase, lessonData.course, requestedSite, { canonicalOnly: true });
        }
    ...
    let { error: insertErr } = await supabase.from("lessons").insert(insertPayload);
        ...
        if (multiSiteEnabled) {
          ...
          await auditLearningSiteOperation(supabase, {
            ...
            action: "lms_multisite_lesson_create",
      ...
      if (action === "update") {
        ...
        if (multiSiteEnabled) {
          await assertCourseInLearningSite(supabase, originalCourse, requestedSite, { canonicalOnly: true });
          await assertCourseInLearningSite(supabase, lessonData.course, requestedSite, { canonicalOnly: true });
        }
        ...
        if (multiSiteEnabled) {
          ...
          await auditLearningSiteOperation(supabase, {
            adminEmail: adminSession.email,
            action: "lms_multisite_lesson_update",
            courseSlug: lessonData.course,
            selectedSite: requestedSite,
            effectiveSite: requestedSite,
      ...
      if (action === "delete") {
        ...
        if (multiSiteEnabled) {
          ...
          await auditLearningSiteOperation(supabase, {
            adminEmail: adminSession.email,
            action: "lms_multisite_lesson_hide",
            courseSlug: course,
            selectedSite: requestedSite,
            effectiveSite: requestedSite,
