...
import {
  ...
  businessTableCounts
} from "../scripts/lib/dual-lms-business-checksum.mjs";
...
test("nullable lms_tenant schema addition leaves business checksum unchanged", () => {
  const before = fixture();
  const after = clone(before);
  for (const row of after.courses) row.lms_tenant = null;
...
test("timestamps and nullable lms_tenant are excluded from business checksum", () => {
  const before = fixture();
  const after = clone(before);
  after.courses[0].created_at = "different";
  after.courses[0].updated_at = "different";
  after.courses[0].lms_tenant = null;
  assert.equal(businessDataChecksum(before), businessDataChecksum(after));
});

test("unexpected explicit lms_tenant fails the independent new-column invariant", () => {
  for (const value of ["yeunauan", "yeubep"]) {
    const rows = fixture();
    rows.courses[0].lms_tenant = value;
    const nonNull = rows.courses.filter((row) => row.lms_tenant !== null).length;
...
test("dual-LMS migration is additive and does not serialize or mutate business rows", () => {
  const sql = fs.readFileSync(
    path.join(ROOT, "migrations/20260730_dual_lms_tenant.sql"),
    "utf8"
  );
  const executableSql = sql.replace(/--.*$/gm, "");
  assert.doesNotMatch(executableSql, /\bSELECT\s+\*/i);
  assert.doesNotMatch(executableSql, /row_to_json|to_jsonb/i);
  assert.doesNotMatch(executableSql, /\bUPDATE\b|\bDELETE\b|\bINSERT\b/i);
  assert.match(sql, /ADD COLUMN IF NOT EXISTS lms_tenant TEXT/);
});
...
test("courses allowlist is exact pre-migration business schema and excludes volatile/new fields", () => {
  ...
  assert.equal(BUSINESS_COLUMN_ALLOWLIST.courses.includes("lms_tenant"), false);
