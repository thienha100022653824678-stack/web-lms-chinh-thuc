...
test("Preview substrate is guarded, service-role-only and outside Production chain", () => {
  const forward = fs.readFileSync(new URL("../migrations/preview/20260730_dual_lms_preview_substrate.sql", import.meta.url), "utf8");
  const rollback = fs.readFileSync(new URL("../migrations/preview/20260730_dual_lms_preview_substrate_rollback.sql", import.meta.url), "utf8");
  const seed = fs.readFileSync(new URL("../migrations/preview/20260730_dual_lms_preview_seed.sql", import.meta.url), "utf8");
  ...
  assert.match(forward, /CREATE TABLE IF NOT EXISTS public\.orders/);
  assert.match(forward, /Synthetic Preview orders only/);
...
test("Every Preview Drive action uses one fail-closed dry-run adapter", () => {
  ...
  assert.match(helper, /assertCourseInLmsTenant/);
  assert.match(helper, /auditLmsTenantOperation/);
...
test("Preview auth route is 404/fail-closed outside Preview and fixed identity only", () => {
  ...
  assert.match(handler, /admin\.dual-lms\.preview@example\.test/);
...
test("Progress admin handler validates canonical course/lesson and verifies persisted row", () => {
  ...
  assert.match(source, /assertCourseInLmsTenant/);
